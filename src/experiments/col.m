...
str_domains = {'1', '2'};
list_acc = [];
mode_new = 'closed'; % closed for MSDI-V , and sgd for MSDI(Adam)
for i = 1 : 2
    src = str_domains{i};
    tar = num2str(3 - i);
...
%            [acc,y_pred,time_pass] = CORAL_SVM(Xs',Ys,Xt',Yt)
% %             model=svmtrain(Ys, Xs')
% %             [predicted_label, Acc_l, decision_values]=svmpredict(Yt, Xt', model);
%             Acc = acc;
%             [Acc,~,~,~] = MEDA(Xs',Ys,Xt',Yt,options);
             [Acc,~,~,~] = MSDI(Xs',Ys,Xt',Yt,options,mode_new,src,tar);
        case 'adam'
            options.d = 20;
            options.rho = 0.1;
            options.p = 10;
